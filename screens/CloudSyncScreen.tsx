...
function CloudSyncScreen() {
  ...
  const applySettings = async () => {
    ...
    const autoSync = parsed.autoSync ?? false;
    const showBadge = parsed.showSyncBadge ?? true;
    setShowSyncBadge(showBadge);
    return { autoSync };
  };

  // Re-apply settings when returning to this tab. Optionally auto-sync.
  useFocusEffect(
    React.useCallback(() => {
      const onFocus = async () => {
        const { autoSync } = await applySettings();
        if (autoSync) {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      {/* Buttons */}
      <CustomButton title="📥 View Notes From Cloud" onPress={pullFromCloud} />
      <CustomButton title="📤 Sync Pending Notes" onPress={syncPendingNotes} />
